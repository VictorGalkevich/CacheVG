...
public class GbitCommandProcessor implements Processor {
    @Override
    public String process(String[] args) {
        if (args.length < 3) {
            String internal = String.join(" ", args);
            return "Command format must be: GET [key] [index], instead got: %s".formatted(internal);
        }
        ...
        return String.valueOf(((DefaultString) obj).getBit(val));
    }
}
