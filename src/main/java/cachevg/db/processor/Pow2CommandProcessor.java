...
public class Pow2CommandProcessor implements Processor {
    @Override
    public String process(String[] args) {
        if (args.length < 2) {
            String internal = String.join(" ", args);
            return "Command format must be: POW2 [key], instead got: %s".formatted(internal);
        }
        ...
        return ((IntegerValue) obj).isPowerOfTwo() ? "TRUE" : "FALSE";
    }
}
