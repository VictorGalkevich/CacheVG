...
public class StrlenCommandProcessor implements Processor {
    @Override
    public String process(String[] args) {
        if (args.length < 3) {
            String internal = String.join(" ", args);
            return "Command format must be: STRLEN [key], instead got: %s".formatted(internal);
        }
        ...
        return String.valueOf(((DefaultString) obj).length());
    }
}
